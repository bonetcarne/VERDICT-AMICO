...
methods
    ...
    % =================================
    function obj = AMICO_VERDICTPROSTATE(k)
        global CONFIG
        ...
        obj.dIC       = 2.0 * 1E-9;
        obj.Rs        = linspace(0.01,15.1,k).*1E-6;
        obj.dEES      = 2.0 * 1E-9;
        obj.P         = 8.0 * 1E-9;
        obj.OUTPUT_names        = {'R', 'fIC', 'fEES', 'fVASC', 'Fobj', 'FobjCamino'};
        obj.OUTPUT_descriptions = {'R', 'fIC', 'fEES', 'fVASC', 'Fobj', 'FobjCamino'};
    ...
    function GenerateKernels( obj, ATOMS_path, schemeHR, AUX, idx_IN, idx_OUT )
        ...
        for R = obj.Rs
            ...
            CMD = sprintf( '%s/datasynth -synthmodel compartment 1 SPHEREGPD %E %E -schemefile %s -voxels 1 -outputfile %s 2> /dev/null', CAMINO_path, obj.dIC, R, schemeHrFilename, filenameHr );
        ...
        % generate
        if exist( filenameHr, 'file' ), delete( filenameHr ); end
        CMD = sprintf( '%s/datasynth -synthmodel compartment 1 BALL %E -schemefile %s -voxels 1 -outputfile %s 2> /dev/null', CAMINO_path, obj.dEES, schemeHrFilename, filenameHr );
        [status result] = system( CMD );
        if status>0
            disp(result)
        ...
        % generate
        if exist( filenameHr, 'file' ), delete( filenameHr ); end
        CMD = sprintf( '%s/datasynth -synthmodel compartment 1 ASTROSTICKS %E -schemefile %s -voxels 1 -outputfile %s 2> /dev/null', CAMINO_path, obj.P, schemeHrFilename, filenameHr );
        [status result] = system( CMD );
        if status>0
            disp(result)
            error( '[AMICO_VERDICTPROSTATE.GenerateKernels] problems generating the signal' );
    ...
    function [ MAPs ] = Fit( obj, y, i1, i2 )
        ...
        % compute MAPS
        xIC = x( 1:size(KERNELS.Aic,2) );
        fIC = sum( xIC );
        MAPs(1) = KERNELS.Aic_R * xIC / ( fIC + eps );           % cell radius
        MAPs(2) = fIC;                                           % fIC
        MAPs(3) = sum(x(size(KERNELS.Aic,2)+1:end-1 ));                                    % fEES
        MAPs(4) = x( end );                                      % fVASC
        ...
        %original_fObj
        sigma = 0.05;
        ymod=sqrt(y_predicted.^2+sigma.^2);
        dy = y - ymod;
        LoffG=sum(dy.^2);
        MAPs(6) =LoffG./sigma(1)^2;
...
%         MAPs(5) = sum( (y - sqrt((y_predicted).^2+sigma^2) ) ).^2;
    ...
    % Fit the model to each voxel
    % ===========================
    function [ x, y_predicted,MAPs] = Fit_xs( obj, y, i1, i2 )
        global CONFIG KERNELS
