...
%
function TIME=AMICO_Fit_xs(nVAS)
    ...
    Xs = zeros( [CONFIG.dim(1:3) nVAS]);
    Ys = zeros( [CONFIG.dim(1:3) CONFIG.dim(4)]);
	...
	if ~isempty(CONFIG.model)
        ...
        for ix = 1:niiSIGNAL.hdr.dime.dim(2)
            ...
            try
                ...
                %b0 modification (variable TE)
                ndir=CONFIG.scheme.dwi_count/CONFIG.scheme.b0_count; %same num directions for b0 assumed
                clear nb0
                for nb=1:CONFIG.scheme.b0_count
                    nb0(CONFIG.scheme.b0_idx(nb):CONFIG.scheme.b0_idx(nb)+ ndir,1) = squeeze( niiSIGNAL.img(ix,iy,iz,CONFIG.scheme.b0_idx(nb)) );
                end
                y = double( squeeze( niiSIGNAL.img(ix,iy,iz,:))  ./ ( nb0 + eps ) );
                y(nb0==0)=0;
                ...
                [xs,y_pred,vox_MAPs] = CONFIG.model.Fit_xs( y, i1, i2 );
    ...
    % =====================================
    %out_put_path=sprintf('%s_k%i',CONFIG.OUTPUT_path,k);
    out_put_path=CONFIG.OUTPUT_path;
    
    mkdir(out_put_path);
    fprintf( '\n-> Saving output to "AMICO/*":\n' );

    fprintf( '\t- CONFIG.mat' );
    save( fullfile(out_put_path,'CONFIG.mat'), '-v6', 'CONFIG' )
    fprintf( ' [OK]\n' );
    ...
    save_untouch_nii( niiMAP, fullfile(out_put_path,'FIT_dir.nii') );
    fprintf( ' [OK]\n' );
    
    save(fullfile(out_put_path,'xs.mat'),'Xs','Ys')
    ...
    for i = 1:numel(CONFIG.model.OUTPUT_names)
        ...
        niiMAP.hdr.dime.calmin = niiMAP.hdr.dime.glmin;
        niiMAP.hdr.dime.calmax = niiMAP.hdr.dime.glmax;
        save_untouch_nii( niiMAP, fullfile(out_put_path,['FIT_' CONFIG.model.OUTPUT_names{i} '.nii']) );
